...
function journalListViewDiscipline() {
  ...
  const rail = Array.from({ length: 28 }, (_, offset) => { const item = new Date(); item.setDate(item.getDate() - offset); const key = item.toISOString().slice(0, 10); return { item, key, selected: key === date, marked: entries.some(entry => entry.date === key) }; }).reverse();
  const pageLines = dayEntries.length ? dayEntries.slice(0, 3).map(entry => `<div class="book-line"><i></i><strong>${esc(entry.block || 'Journal')}</strong><em>${esc(entry.answer || 'Okay')}</em></div>`).join('') : '<p class="book-empty">The page stayed blank.<br>The book remembers the gaps too.</p>';
  const cards = dayEntries.length ? dayEntries.map(entry => { const category = entry.block === 'Journal' || entry.block?.includes('Close') ? 'REFLECTION' : 'GROWTH'; const timestamp = entryTimestamp(entry); return `<article class="journal-card"><div class="journal-card-top"><span class="entry-tile">${category === 'GROWTH' ? '&#8767;' : '&#9728;'}</span><div class="entry-meta"><span>${category}</span>${timestamp ? `<time>${timestamp}</time>` : ''}</div></div><strong>${esc(entry.block || 'A kept line')}</strong>${entry.question ? `<p class="entry-question">${esc(entry.question)}</p>` : ''}<p class="entry-answer">${entry.note ? esc(entry.note) : esc(entry.answer || 'Okay')}</p></article>`; }).join('') : '<div class="journal-empty">No entry for this day yet. A blank page still belongs in the book.</div>';
...
const screens = {
  ...
  jcompose: journalComposeSheet, schedule: schedulePageView, stats: statsPageViewDiscipline, settings: settingsPageViewV4,
...
function loadDemo() {
  ...
  state.journal = Array.from({ length: 14 }, (_, index) => ({ date: dayAt(index).toISOString().slice(0, 10), createdAt: dayAt(index).toISOString(), block: blocks[(index + 2) % blocks.length]?.name || 'Deep study', answer: index % 4 === 0 ? 'Partly' : 'Yes', mood: moods[index % moods.length], note: notes[index % notes.length] }));
  state.tasks = [
