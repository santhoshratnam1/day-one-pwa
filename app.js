...
function celebrateView() { const block = state.blocks.find(item => item.id === state.activeBlock); const done = state.blocks.filter(item => item.done).length; return `<main class="celebrate-page"><div class="celebrate-blob"></div><div class="celebrate-content"><div class="cel-kicker">BLOCK COMPLETE</div><div class="cel-t">${state.dayWon ? 'Kept the promise.' : 'Done.'}</div><p class="cel-s">${state.dayWon ? 'The shape held. That is worth keeping.' : 'One block met, one promise kept small.'}</p><div class="cel-proof"><span>${esc(block?.time || 'NOW')}</span><strong>${esc(block?.name || 'One useful block')}</strong><small>${done} of ${state.blocks.length || 7} blocks held today</small></div><button class="cel-hint" data-celebrate-next>continue to today <span aria-hidden="true">→</span></button></div></main>`; }
...
function checkinViewDiscipline() {
  const answered = Boolean(state.checkinAnswered);
  const source = state.checkin?.source === 'gpt-5.6' ? 'GPT-5.6' : 'OFFLINE';
  return `<main class="checkin-page"><div class="ci-wash"></div><div class="ci-content"><div class="ci-tag">CHECK-IN · ${source}</div><h1 class="ci-q">${esc(state.checkin?.question || fallbackQuestions[0])}</h1><p class="ci-lede">Choose the closest answer. You can leave one plain line after.</p><div class="ci-answers">${['Yes', 'Partly', 'Not today'].map(answer => `<button class="${state.checkinAnswer === answer ? 'selected' : ''}" data-ci-answer="${answer}">${answer}</button>`).join('')}</div>${answered ? `<textarea class="ci-input ci-note-reveal" id="ci-note" placeholder="add a note (optional)">${esc(state.checkinNote || '')}</textarea><div class="ci-bottom"><button class="ci-log" data-ci-log>Log it →</button></div>` : ''}</div></main>`;
}
